...
//
//https://github.com/debugly/JSONUtil

#import <Foundation/Foundation.h>

@protocol SLAnalyzeJSON2ModelProtocol <NSObject>

@optional;
/**
 *  @brief:  存放冲突key映射的字典 @{@"id":@"Newsid"}
 */
- (NSDictionary *)sl_collideKeysMap;

/**
 *  @brief:  存放冲突key映射Model的字典，@{@"courses":@"CourseModel"}
 */
- (NSDictionary *)sl_collideKeyModelMap;

/**
 *  @brief:  将json赋值给model后会调用model对象的该方法，可以再次做Value转换；
 */
- (void)sl_valueNeedTransfer;

@end


@interface NSObject (SLAnalyzeJSON2Model)<SLAnalyzeJSON2ModelProtocol>
 ...
 */
- (void)sl_assembleDataFormDic:(NSDictionary *)jsonDic;
 ...
 */
+ (instancetype)sl_instanceFromValue:(id)value;
 ...
 */
FOUNDATION_EXPORT id SLJSON2Model(id Json,NSString *modelName);
/**
 *  @brief 根据 keyPath 找到目标 JSON，可辅助 JSON2Model 函数使用，先找到 JOSN 在解析；
 *  @param keyPath JSON里的一个路径；一个以 / 分割的字符串；例如：@"data" , @"data/list";
 */
FOUNDATION_EXPORT id SLFindJSONwithKeyPath(NSString *keyPath,NSDictionary *JSON);

/**
 * @brief 如果没有使用SLJSON2Model方法，而是使用了findJSONwithKeyPath_SL方法，那么JOSN里面不全是String，可以通过该方法解决；
 */
FOUNDATION_EXPORT id SLJSON2StringJOSN(id findJson);
